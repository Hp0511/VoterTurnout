...
primaryEntropy <- primaryEntropy %>%
  rename(
    ...
    "rep_primary_total" = "REP_primary_total",
    "dem_primary_total" = "DEM_primary_total",
  )

primaryEntropy <- primaryEntropy %>%
  # Remove the general_total column
  select(-general_total) %>%
  
  # Create a new column 'dem_general_total' which sums all columns starting with 'dem_gen'
  mutate(
    dem_general_total = rowSums(select(., starts_with("dem_gen")), na.rm = TRUE),
    
    # Create a new column 'rep_general_total' which sums all columns starting with 'rep_gen'
    rep_general_total = rowSums(select(., starts_with("rep_gen")), na.rm = TRUE)
  )
...
```{r}
write.csv(primaryEntropy, "OH.csv", row.names = FALSE, quote = FALSE)
```
